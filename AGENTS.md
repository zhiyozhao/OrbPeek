...
- The composited capture needs the window on screen, so fake docks **capture first, park after** (the `.docking` phase — 300ms timeout parks regardless). A **hidden re-dock never moves the window** (a capture would flash it on screen): it re-parks at the new edge and keeps the strip image (same edge) or a placeholder (new edge); the next dockBack captures fresh. Slice content mimics the window really sliding off the edge: the **opposite** edge stays visible (up → bottom slice, down → title bar, left → right edge, right → left edge — same as the real slivers' trailing 6px).
